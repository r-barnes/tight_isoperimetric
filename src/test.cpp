...
TEST_CASE("area of four circles joined by rectangle (1 hole)") {
  ...
  const auto rect1 = construct_polygon(Point_2(1, 0), Point_2(5, 0), Point_2(5, 2), Point_2(1, 2));
  const auto rect2 = construct_polygon(Point_2(1, 4), Point_2(5, 4), Point_2(5, 6), Point_2(1, 6));
  const auto rect3 = construct_polygon(Point_2(0, 1), Point_2(2, 1), Point_2(2, 5), Point_2(0, 5));
  const auto rect4 = construct_polygon(Point_2(4, 1), Point_2(6, 1), Point_2(6, 5), Point_2(4, 5));
  S.join(rect1);
  S.join(rect2);
  S.join(rect3);
  S.join(rect4);
...
TEST_CASE("area of three overlapping circles") {
  Polygon_set_2 S;
  const auto circ1 = construct_polygon(Point_2(0, 0), 1);
  const auto circ2 = construct_polygon(Point_2(0.5, 0), 1);
  const auto circ3 = construct_polygon(Point_2(-1.8, 0), 1);
  S.join(circ1);
  S.join(circ2);
  S.join(circ3);
  CHECK(S.number_of_polygons_with_holes() == 1);

  CHECK(CGAL::to_double(area(S)) == doctest::Approx(3 * M_PI - lens_area(1, 1, 0.5) - lens_area(1, 1, 1.8)));
}
