...
// -frounding-math main.cpp -lgmpxx -lmpfr -lgmp
...
std::vector<Vertex_handle> map_to_ordered_vector(const VH_Int_Map& m) {
  ...
  std::sort(to_sort.begin(), to_sort.end(), [](const auto& a, const auto& b) { return a.second < b.second; });

  std::vector<Vertex_handle> ret;
  ret.reserve(to_sort.size());
  std::transform(begin(to_sort), end(to_sort), std::back_inserter(ret), [](auto const& pair) { return pair.first; });
...
std::set<Vertex_handle> identify_vertex_handles_inside_mp(const VoronoiDiagram& vd, const MultiPolygon& mp) {
  ...
  for (auto edge_iter = vd.bounded_halfedges_begin(); edge_iter != vd.bounded_halfedges_end(); edge_iter++) {
    ...
    const auto vertex_in_mp_interior = [&](const Vertex_handle& vh) {
      ...
      const auto inside_of_a_poly = std::any_of(
          mp.begin(), mp.end(), [&](const auto& poly) { return inside(CGAL::oriented_side(vh->point(), poly)); });
...
Point2_Set identify_concave_points_of_mp(const MultiPolygon& mp) {
  ...
  const auto consider_polygon = [&](const auto& poly, const double sense) {
    for (size_t i = 0; i < poly.size() + 1; i++) {
      const auto zcp =
          z_cross_product(poly[(i + 0) % poly.size()], poly[(i + 1) % poly.size()], poly[(i + 2) % poly.size()]);
      if (sense * zcp < 0) {
...
void print_medial_axis_edges(const MedialData& md, const std::string& filename) {
  ...
  for (std::size_t i = 0; i < md.edges.size(); i++) {
    fout << md.edges[i].first << "," << md.edges[i].second << "," << md.ups[i]->is_point()
         << ","                      // Is up-governor a point?
...
MedialData filter_voronoi_diagram_to_medial_axis(const VoronoiDiagram& vd, const MultiPolygon& mp) {
  ...
  const auto pconcave = [&](const Point_2& pt) { return concave_points.count(pt) != 0; };
...
MedialGraph get_medial_axis_graph(const MedialData& md) {
  ...
  for (size_t e = 0; e < md.edges.size(); e++) {
    ...
    spt.squared_distance = point_to_min_site_distance(spt.pt, md.ups.at(e)->site(), md.downs.at(e)->site());
    auto& ept            = mg.node_prop(mg.find(edge.second));
    ept.squared_distance = point_to_min_site_distance(ept.pt, md.ups.at(e)->site(), md.downs.at(e)->site());
    ...
    for (int t = 1; t < STEPS; t++) {
      ...
      mg.add_node_with_prop(
          ...
          MedialPoint{
              ...
              .squared_distance =
                  point_to_min_site_distance(interp_point, md.ups.at(e)->site(), md.downs.at(e)->site())});
...
void iterate_graph(const MedialGraph& mg) {
  ...
  const auto sdist = [&](const auto& node) { return mg.node_prop(node).squared_distance; };
  ...
  // edge
  {
    ...
    for (std::size_t i = 0; i < ma_data.ups.size(); i++) {
      ...
      if (s1.is_point()) {
        ...
        fout << "Line(" << s1.segment().source().x() << "," << s1.segment().source().y() << ","
             << s1.segment().target().x() << "," << s1.segment().target().y() << ") ";
      }
      ...
      if (s2.is_point()) {
        ...
        fout << "Line(" << s2.segment().source().x() << "," << s2.segment().source().y() << ","
             << s2.segment().target().x() << "," << s2.segment().target().y() << ")";
      }
